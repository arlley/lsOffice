<template>
  <div class="w-100">
    <tableNav localName="增加会议安排" showFirstBtn="true" firstBtnName="返回" :firstCallBack="putOff"></tableNav>
    <div class="mx-auto pt-4" style="width:800px">
      <form class="m-75 mx-auto">
        <div class="pt-sm-3" v-for="(value,key,index) in meeting" :key="index">
          <div class="form-group row" v-if="key!='content'">
            <label
...
export default {
  ...
  methods: {
    putIn() {
      if (this.isRight) {
        req.POST(url.meeting.insert, req.myInsert(this.meeting)).then(data => {
