<template>
  <div class="w-100">
    <tableNav localName="修改行程" showFirstBtn="true" firstBtnName="返回" :firstCallBack="putOff"></tableNav>
    <div class="mx-auto pt-4" style="width:800px">
      <form class="m-75 mx-auto">
        <div class="pt-sm-3" v-for="(value,key,index) in schedule" :key="index">
          ...
          <div class="form-group row" v-else-if="key=='date'">
            <label
...
export default {
  ...
  data() {
    return {
      ...
      schedule: {
        date: "",
        plan: this.$store.state.schedule.plan,
        year: "",
        month: "",
        day: "",
        username: this.$store.state.enrollee,
        id:this.$store.state.schedule.id
      },
      inputGroup: [
        {
          title: "计划时间",
  ...
  methods: {
    putIn() {
      if (this.isRight) {
        ...
        req
          .POST(url.schedule.update, req.myUpdate(this.schedule))
          .then(data => {
