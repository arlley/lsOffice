  ...
  <div>
    ...
    <div class="components-form-demo-advanced-search" style="padding-top: 70px">
      ...
      <div class="search-result-list" style="text-align: left">
        <span >律所收支统计 收入{{payDetailVo.income}} 收入分类:{{payDetailVo.incomeStatistics}} 支出{{payDetailVo.outCome}} 支出分类{{payDetailVo.outComeStatistics}}</span>
      </div>
    </div>
  </div>
</template>
<script>
    ...
    const columns = [{
        ...
        mounted(){
            ...
            req.GET("/pay/detail/index", null, function (response) {
        ...
        methods: {
            search(){
                ...
                this.$refs['ruleForm'].validate(valid =>{
                    if(valid){
                        ...
                        req.POST("/pay/detail/search", scope.$data.searchData, function (response) {
            ...
            addIncome(){
                this.$router.push({name:'AddLawyerPay'})
            },
            addOutcome(){
                this.$router.push({name:'AddLawyerOut'})
            }
        }
    };
</script>
