          ...
          <div class="row">
            ...
            <div class="col-sm-9 col-sm-offset-2">
              <a-form-model-item has-feedback label="收支时间" prop="time">
                <a-date-picker format = "YYYY-MM-DD" v-model="pay.time"  @change="onChange" placeholder="请选择收支时间" style="width:100%; min-width: 100%"/>
              </a-form-model-item>
            </div>
          </div>
          <div class="row">
        ...
        mounted(){
            ...
            req.GET("code/getCodesByType", {codeType: 'income'}, function (response) {
                ...
                req.GET("code/getCodesByType", {codeType: 'payType'}, function (response) {
                    ...
                    req.GET("pay/getPay", {'id': id}, function (response) {
                        scope.pay = response.data.data;
                        scope.pay.timeShow = scope.pay.time;
                    })
                });
            });
        ...
        methods: {
            putIn() {
                ...
                this.$refs['payForm'].validate(valid => {
                    req.POST('pay/update', this.pay, function (response) {
                        scope.$router.push({
                            name: 'Success',
                            query: {
                                message: "修改收支成功",
            ...
            onChange:function (date, dateString) {
                ...
                this.$data.pay.timeShow = dateString;
            }
        }
    };
</script>
