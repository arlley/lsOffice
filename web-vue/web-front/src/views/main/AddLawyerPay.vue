<template>
  <div class="w-100">
    <tableNav localName="添加律所收入" showFirstBtn="true" firstBtnName="返回" :firstCallBack="putOff"></tableNav>
        ...
        <div class="pt-sm-3">
          <div class="row">
            ...
            <div class="col-sm-9 col-sm-offset-2">
              <a-form-model-item has-feedback label="收入类别" prop="payType">
                <a-select v-model="pay.payType" placeholder="请选择收支类别">
          ...
          <div class="row" v-if="pay.payType == '3'">
            <div class="col-sm-1"></div>
            <div class="col-sm-9 col-sm-offset-2">
              <a-form-model-item has-feedback label="类别手填" prop="payTime">
          ...
          <div class="row">
            <div class="col-sm-1"></div>
            <div class="col-sm-9 col-sm-offset-2">
              <a-form-model-item has-feedback label="收入时间" prop="payTime">
          ...
          <div class="row">
            <div class="col-sm-1"></div>
            <div class="col-sm-9 col-sm-offset-2">
              <a-form-model-item has-feedback label="收支金额" prop="payAmount">
    ...
    export default {
        ...
        data() {
            return {
                isRight: true,
                rules:{
                    payType:[{required:true, message:'收入类别不能为空', trigger: 'change'}],
                    payTime:[{required:true, message:'收支时间不能为空', trigger: 'change'}],
                    payAmount:[{required:true, message:'收支金额不能为空', trigger: 'change'},
                        { validator: this.$verify.formatNum, trigger: 'change' }],
                    payWay:[{required:true, message:'收支方式不能为空', trigger: 'change'}],
                },
                layout: {
                    labelCol: { span: 4 },
                    wrapperCol: { span: 14 },
        ...
        methods: {
            putIn() {
                ...
                this.$refs['payForm'].validate(valid => {
                    req.POST('pay/detail/insertLawyer', this.pay, function (response) {
                        scope.$router.push({
                            name: 'Success',
                            query: {
                                message: "增加律所收入成功",
